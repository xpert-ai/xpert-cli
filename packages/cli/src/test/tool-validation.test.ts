...
describe("tool payload validation", () => {
  ...
  it("returns a readable validation error before permission or execute", async () => {
    ...
    expect(String(toolMessages[0]?.content)).toContain("Invalid arguments for Read");
    expect(toolMessages[0]?.artifact).toMatchObject({
      code: "INVALID_TOOL_PAYLOAD",
      toolName: "Read",
    });
    expect(toolMessages[0]?.artifact?.issues?.[0]).toMatchObject({
      field: "path",
      expected: "a non-empty string path inside the project root",
    });

    expect(nextSession.turns).toHaveLength(1);
    expect(nextSession.turns[0]?.toolEvents[0]).toMatchObject({
      callId: "call-invalid",
      status: "error",
      code: "INVALID_TOOL_PAYLOAD",
    });
